...
class Dashboard extends Component {
  ...
  exportToText = (landno) => async () => {
    // Define the data you want to export to a text file
    const info1 = "Land Ownership Hierarchy\n";
    const info2 = await this.state.LandInstance.methods.TrackLand(landno).call();
    const data = info1.concat(info2);
    // Create a Blob object from the text data
    ...
    // Clean up the link and URL objects
...
  }
  ...
  redirect = (land_id) => async () => {
    var arg1 = "argument1";
    var arg2 = "argument2";
    var url = "previous_owners.html";
    ...
    try {
      ...
      for (var i = 0; i < count; i++) {
        ...
        row.push(<tr><td>{i + 1}</td><td>{rowsArea[i]}</td><td>{rowsCity[i]}</td><td>{rowsState[i]}</td><td>{rowsPrice[i]}</td><td>{rowsPID[i]}</td><td>{rowsSurvey[i]}</td>
          <td>
            <Button onClick={this.exportToText(i+1)} className="button-vote">
              Previous Owners
            </Button>
          </td>
          <td>
